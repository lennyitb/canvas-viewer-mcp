...
Requires the `canvas-viewer` MCP server.

## 1. Gather (budgeted)

Load every Canvas tool you'll need in ONE `tool_search` (query "canvas",
limit 12), not one search per tool.

A normal run is at most these four calls:

1. **Clock**: current local time and zone from the device clock tool if one
   exists. If there is no clock tool, use the timezone the user has already
   stated; if none is known, say in one short line which zone you assumed.
